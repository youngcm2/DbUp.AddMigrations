...
namespace DbUp.AddMigrations
{
    class FileProviderMigrations : IProviderMigrations<FileProviderOptions>
    {
        ...
        public int Add(FileProviderOptions options)
        {
            ...
            var scriptFilePath = Path.Combine(scriptsPath, fileName);
            ...
            try
            {
                File.WriteAllText(scriptFilePath, header.ToString());
            }
            catch (Exception exception)
            {
                _logger.LogCritical($"Could not create file: {scriptFilePath} - {exception.Message}");
                return 1;
            }
